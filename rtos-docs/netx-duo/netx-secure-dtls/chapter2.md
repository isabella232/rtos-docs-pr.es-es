...
ms.locfileid: "116784094"
---
# <a name="chapter-2-installation-and-use-of-azure-rtos-netx-secure-dtls"></a>Capítulo 2: Instalación y uso de DTLS de Azure RTOS NetX Secure

Este capítulo contiene una descripción de varios problemas relacionados con la instalación, la configuración y el uso del componente DTLS de Azure RTOS NetX Secure.
...
- **nx_secure_dtls_api.h**: archivo de encabezado de la API pública para NetX Secure DTLS
- **nx_secure_tls_user.h**: el usuario define el archivo de encabezado para NetX Secure DTLS
- **nx_secure_port.h**: definiciones específicas de la plataforma para NetX Secure
- **nx_secure_dtls.h**: archivo de encabezado para NetX Secure DTLS
- **nx_secure_tls.h**: archivo de encabezado para NetX Secure TLS
- **nx_secure_dtls\*.c/h**: archivos de código fuente C/H para NetX Secure DTLS
- **nx_secure_tls\*.c/h**: archivos de código fuente C/H para NetX Secure TLS
- **nx_crypto\*.c/h**: archivos de código fuente C/H para NetX Secure Cryptography
- **nx_secure_x509\*.c/h**: archivos de código fuente C/H para certificados digitales X.509
- **demo_netx_secure_dtls.c**: archivo de código fuente C para demostración de NetX Secure DTLS
- **NetX_Secure_DTLS_User_Guide.pdf**: descripción en PDF del producto NetX Secure
...
## <a name="netx-secure-dtls-installation"></a>Instalación de NetX Secure DTLS

Para usar NetX Secure DTLS, toda la distribución que se menciona anteriormente debe copiarse en el mismo directorio en el que se ha instalado NetX. Por ejemplo, si NetX está instalado en el directorio “ *\threadx\arm7\NetX*”, los directorios *nx_secure\*.\** deben copiarse en “ *\threadx\arm7\NetXSecure*”.

## <a name="using-netx-secure-dlts"></a>Uso de NetX Secure DLTS

El uso de NetX Secure DTLS es sencillo. El código de la aplicación debe incluir *nx_secure_dtls_api.h* después de incluir *tx_api.h* y *nx_api.h* (dedicados a ThreadX y NetX, respectivamente). Después de incluir *nx_secure_dtls_api.h*, el código de la aplicación puede realizar las llamadas de función de NetX Secure DTLS especificadas más adelante en este manual. La aplicación también debe importar los archivos *nx_secure\*.\** en una biblioteca de NetX Secure, y los archivos *nx_crypto\*.\** específicos de la plataforma en una biblioteca de NetXCrypto, que luego se vinculan al archivo binario de la aplicación final.

## <a name="small-example-system-dtls-client"></a>Sistema de ejemplo pequeño (cliente DTLS)

Un ejemplo de lo fácil que es usar NetX Secure DTLS se describe en la figura 1.1, que aparece a continuación y muestra un cliente de DTLS simple, diseñado para funcionar con un servidor de DTLS de OpenSSL (o similar). Observe que la estructura del programa del cliente de DTLS es muy similar a la de un cliente NetX Secure TLS (consulte la documentación de NetX Secure TLS). Esto se debe a que el protocolo DTLS es esencialmente una versión de TLS para su uso a través de protocolos de red de transporte no confiables, como UDP.
...
## <a name="small-example-system-dtls-server"></a>Sistema de ejemplo pequeño (servidor DTLS)

Un ejemplo de lo fácil que es usar NetX Secure se describe en la figura 1.2, que aparece a continuación y muestra un servidor DTLS sencillo. Observe que la funcionalidad del servidor de DTLS es bastante diferente del cliente de DTLS y el cliente/servidor de TLS, ya que el servidor de DTLS tiene que administrar varias solicitudes de cliente entrantes en un solo puerto UDP (almacenado en la instancia del servidor de DTLS).
...
**Figura 1.2 Ejemplo de servidor NetX Secure DTLS**

## <a name="configuration-options"></a>Opciones de configuración

Hay varias opciones de configuración para compilar NetX Secure.
A continuación, se muestra una lista de todas las opciones, donde se describe con detalle cada una de ellas:

| Definir                                                 | Significado                                                                                                                                                                                                                |
|--------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **NX_SECURE_ENABLE_DTLS**                           | Esta macro debe definirse para habilitar la lógica de DTLS en NetX Secure.                                                                                                                                                       |
...
| **NX_SECURE_TLS_CLIENT_DISABLED**                  | Si se define, esta opción quita todo el código de pila TLS/DTLS relacionado con el modo de cliente, lo que reduce el uso de código y datos.                                                                                                            |
| **NX_SECURE_TLS_SERVER_DISABLED**                  | Si se define, esta opción quita todo el código de pila TLS/DTLS relacionado con el modo de servidor, lo que reduce el uso de código y datos.                                                                                                            |
...
| **NX_SECURE_X509_STRICT_NAME_COMPARE**            | Si se define, esta opción habilita la comparación estricta de nombres distintivos para los certificados X.509 en la búsqueda y comprobación de certificados. De forma predeterminada, solo se comparan los campos nombre común de los nombres distintivos. |
