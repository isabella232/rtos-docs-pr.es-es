...
ms.locfileid: "116798034"
---
# <a name="chapter-2---installation-and-use-of-azure-rtos-netx-duo-ptp-client"></a>Capítulo 2: Instalación y uso del cliente PTP de Azure RTOS NetX Duo

Este capítulo contiene una descripción de varios problemas relacionados con la instalación, la configuración y el uso del componente cliente PTP de NetX Duo.

## <a name="product-distribution"></a>Distribución del producto

Azure RTOS NetX Duo se puede obtener desde el repositorio de código fuente público en https://github.com/azure-rtos/netxduo/tree/master/addons/ptp.

***nxd_ptp_client.h*** Archivo de encabezado del cliente PTP para NetX Duo ***nxd_ptp_client.c*** Archivo de código fuente C del cliente PTP para NetX Duo ***demo_netx_duo_ptp_client.c*** Demostración del cliente PTP para NetX Duo


## <a name="using-ptp-client"></a>Uso del cliente PTP
Es fácil usar el cliente PTP para NetX Duo. Básicamente, el código de la aplicación debe incluir ***nxd_ptp_client.h** _ después de incluir _*_tx_api.h_*_ y _*_nx_api.h_*_ a fin de usar ThreadX y NetX Duo, respectivamente. Después de incluir el archivo de encabezado del cliente PTP, el código de la aplicación puede realizar las llamadas de función de cliente PTP que se especifican más adelante en esta guía. La aplicación también debe incluir _ *_nxd_ptp_client.c_** en el proceso de compilación. Este archivo se debe compilar de la misma manera que otros archivos de aplicación y su formulario de objetos se debe vincular junto con los archivos de la aplicación. Esto es todo lo que se necesita para usar el cliente PTP de NetX Duo.

## <a name="small-example-system"></a>Pequeño sistema de ejemplo
En la figura 1 que aparece a continuación, se muestra un ejemplo de cómo usar los servicios de cliente PTP de NetX Duo.
```C
/*
...
Hay varias opciones de configuración con el cliente PTP de NetX Duo. A continuación, se muestra una lista de todas las opciones descritas de manera detallada:
* **NX_PTP_CLIENT_THREAD_TIME_SLICE** Define la porción de tiempo del subproceso del cliente PTP. El valor predeterminado es ninguna porción de tiempo.
* **NX_PTP_CLIENT_TIMER_TICKS_PER_SECOND** Define la frecuencia del temporizador interno del cliente PTP. El valor predeterminado es 10, que indica un temporizador de 100 ms.
* **NX_PTP_CLIENT_ANNOUNCE_RECEIPT_TIMEOUT** Define el número máximo de paquetes de anuncio faltantes antes de que se agote el tiempo de espera. El valor predeterminado es 3.
* **NX_PTP_CLIENT_LOG_ANNOUNCE_INTERVAL** Define el intervalo de tiempo entre paquetes de anuncio sucesivos, expresado como logaritmo en base 2. Este valor debe ser uniforme en todo el dominio. El valor predeterminado es 1, es decir, 2 s.
* **NX_PTP_CLIENT_DELAY_REQ_INTERVAL** Define el intervalo de envío de paquetes de solicitudes de retraso. El valor predeterminado es 2 segundos.
* **NX_PTP_CLIENT_MAX_QUEUE_DEPTH** Define la longitud máxima de la cola para el socket de cliente. El valor predeterminado es 5.