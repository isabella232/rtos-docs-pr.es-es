---
title: 'Apéndice D: Ejemplo de servidor DHCPv6 avanzado Azure RTOS NetX Duo'
description: Este capítulo contiene un ejemplo del servidor DHCPv6 avanzado NetX Duo
author: philmea
ms.author: philmea
ms.date: 06/08/2020
ms.topic: article
...
ms.locfileid: "104814753"
---
# <a name="appendix-d----azure-rtos-netx-duo-advanced-dhcpv6-server-example"></a><span data-ttu-id="92351-103">Apéndice D: Ejemplo de servidor DHCPv6 avanzado Azure RTOS NetX Duo</span><span class="sxs-lookup"><span data-stu-id="92351-103">Appendix D -  Azure RTOS NetX Duo Advanced DHCPv6 server example</span></span>

<span data-ttu-id="92351-104">Se trata de un servidor DHCPv6 avanzado que muestra cómo guardar y recuperar la tabla de concesiones de direcciones IP del servidor y las tablas de registros de cliente desde memoria no volátil, como requiere la publicación RFC 3315.</span><span class="sxs-lookup"><span data-stu-id="92351-104">This is an advanced DHCPv6 Server which demonstrates saving and retrieving the Server’s IP address lease table and Client record tables from non volatile memory, as required by the RFC 3315.</span></span>

<span data-ttu-id="92351-105">En este ejemplo, el archivo de inclusión *nxd_dhcpv6_server.h* se incluye en la línea 7.</span><span class="sxs-lookup"><span data-stu-id="92351-105">In this example, the include file *nxd_dhcpv6_server.h* is brought in at line 7.</span></span> <span data-ttu-id="92351-106">A continuación, se crea el subproceso de aplicación de servidor DHCPv6 NetX Duo en la línea 81 en el código de ejemplo siguiente.</span><span class="sxs-lookup"><span data-stu-id="92351-106">Next, the NetX Duo DHCPv6 Server application thread is created at line 81 in the example code below.</span></span> <span data-ttu-id="92351-107">Tenga en cuenta que el bloque de control DHCPv6 “*dhcp_server_0*” se definió anteriormente como una variable global en la línea 19.</span><span class="sxs-lookup"><span data-stu-id="92351-107">Note that the DHCPv6 control block “*dhcp_server_0*” was defined as a global variable at line 19 previously.</span></span>

<span data-ttu-id="92351-108">Antes de crear la instancia del servidor DHCPv6 NetX Duo, la demostración crea un grupo de paquetes para enviar mensajes DHCPv6 en la línea 84, crea una interfaz de subprocesos IP en la línea 102 y habilita UDP en NetX Duo en la línea 116.</span><span class="sxs-lookup"><span data-stu-id="92351-108">Before creating the NetX Duo DHCPv6 Server instance, the demo creates packet pool for sending DHCPv6 messages in line 84, creates an IP thread interface in line 102, and enables UDP in NetX Duo in line 116.</span></span>

<span data-ttu-id="92351-109">La creación correcta del servidor DHCPv6 NetX Duo en la línea 136 incluye las dos funciones de devolución de llamada opcionales que se describen en el capítulo 1. Posibilita IPv6 e ICMPv6 necesarios para que NetX Duo procese las operaciones de IPv6 y DHCPv6 en las líneas 162 y 163.</span><span class="sxs-lookup"><span data-stu-id="92351-109">The successful creation of NetX Duo DHCPv6 Server in line 136 includes the the two optional callbacks function described in Chapter 1.It enables IPv6 and ICMPv6 necessary for NetX Duo to process IPv6 and DHCPv6 operations in line 162-163.</span></span> <span data-ttu-id="92351-110">Antes de que el subproceso del servidor DHCPv6 esté listo para ejecutarse, dicho servidor debe validar su dirección IPv6 (líneas 167 a 180) y definir su interfaz DHCPv6 en las líneas 208 y 209.</span><span class="sxs-lookup"><span data-stu-id="92351-110">Before the DHCPv6 Server thread is ready to run, the DHCPv6 server must validate its IPv6 address(167-180),and define its DHCPv6 interface in lines 208-209.</span></span> <span data-ttu-id="92351-111">Se llama al servicio *nx_dhcpv6_set_server_duid* para crear el servidor si no se ha creado previamente ningún DUID de servidor en la línea 266.</span><span class="sxs-lookup"><span data-stu-id="92351-111">The *nx_dhcpv6_set_server_duid* service is called to create the Server if no Server DUID has been previously created in line 266.</span></span> <span data-ttu-id="92351-112">El servidor configura un intervalo de direcciones IP para crear una lista de direcciones asignables.</span><span class="sxs-lookup"><span data-stu-id="92351-112">The Server sets up an IP address range for creating a list of assignable addresses.</span></span> <span data-ttu-id="92351-113">Si los datos se guardan de una sesión anterior, recupera los registros de cliente y los datos de concesión IPv6 de la memoria en las líneas 283 a 318.</span><span class="sxs-lookup"><span data-stu-id="92351-113">If data is saved from a previous session, it retrieves Client records and IPv6 lease data from memory in lines 283-318.</span></span> <span data-ttu-id="92351-114">También crea su DUID de servidor o, si se ha creado previamente, recupera los datos de DUID del almacenamiento especificado por el usuario.</span><span class="sxs-lookup"><span data-stu-id="92351-114">It also creates its Server DUID, or if one was previously created, retrieves the DUID data from user specified storage.</span></span> <span data-ttu-id="92351-115">Esto es necesario para reproducir un DUID de servidor coherente en los reinicios.</span><span class="sxs-lookup"><span data-stu-id="92351-115">This is necessary to reproduce a consistent Server DUID across reboots.</span></span> <span data-ttu-id="92351-116">Opcionalmente, la aplicación host define un servidor DNS para los clientes que solicitan la configuración del servidor DNS.</span><span class="sxs-lookup"><span data-stu-id="92351-116">Optionally the host application defines a DNS server for Clients requesting DNS server configuration.</span></span>

<span data-ttu-id="92351-117">Después, el host inicia el servidor DHCPv6 en la línea 329.</span><span class="sxs-lookup"><span data-stu-id="92351-117">Next, the host starts the DHCPv6Server in line 329.</span></span> <span data-ttu-id="92351-118">Esto crea el socket UPD del servidor DHCPv6 y activa los temporizadores del servidor DHCPv6 NetX Duo.</span><span class="sxs-lookup"><span data-stu-id="92351-118">This creates the DHCPv6 Server UPD socket and activates NetX Duo DHCPv6 Server timers.</span></span> <span data-ttu-id="92351-119">A continuación, el servidor espera para recibir las solicitudes de cliente.</span><span class="sxs-lookup"><span data-stu-id="92351-119">Then the Server waits to receive Client requests.</span></span> <span data-ttu-id="92351-120">Aunque puede atender a muchos clientes, solo puede procesar una única solicitud de cliente a la vez.</span><span class="sxs-lookup"><span data-stu-id="92351-120">While it can service many Clients it can only process a single Client request at a time.</span></span>

<span data-ttu-id="92351-121">El resto del ejemplo contiene implementaciones de host para guardar y recuperar tablas de servidor de su grupo de direcciones IPv6 asignables y registros de cliente a y desde memoria no volátil, respectivamente.</span><span class="sxs-lookup"><span data-stu-id="92351-121">The remainder of the example contains host implementations for saving and retrieving Server tables of its assignable IPv6 address pool and Client records to and from non volatile memory respectively.</span></span> <span data-ttu-id="92351-122">También contiene un controlador de opciones para las opciones que los clientes DHCPv6 solicitan que no son compatibles directamente con el servidor DHCPv6 NetX Duo (actualmente se admite la opción de servidor DNS).</span><span class="sxs-lookup"><span data-stu-id="92351-122">It also contains an option handler for options requested by DHCPv6 Clients that are not supported directly by the NetX Duo DHCPv6 Server (only the DNS server option is currently supported).</span></span> <span data-ttu-id="92351-123">Por último, hay un código para mostrar cómo guardar y recuperar "tiempo no volátil" mediante el que el servidor realiza un seguimiento de la expiración de la concesión de direcciones IP asignada.</span><span class="sxs-lookup"><span data-stu-id="92351-123">Lastly there is a code for demonstrating how to save and retrieve ‘non volatile time’ by which the Server keeps track of assigned IP lease expiration.</span></span>
...
<span data-ttu-id="92351-124">**Figura 6. Aplicación de servidor DHCPv6 avanzado NetX Duo**</span><span class="sxs-lookup"><span data-stu-id="92351-124">**Figure 6. Advanced NetX Duo DHCPv6 Server Application**</span></span>