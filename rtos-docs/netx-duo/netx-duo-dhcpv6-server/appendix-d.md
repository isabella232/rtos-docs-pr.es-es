...
---
# <a name="appendix-d----azure-rtos-netx-duo-advanced-dhcpv6-server-example"></a>Apéndice D: Ejemplo de servidor DHCPv6 avanzado Azure RTOS NetX Duo

Se trata de un servidor DHCPv6 avanzado que muestra cómo guardar y recuperar la tabla de concesiones de direcciones IP del servidor y las tablas de registros de cliente desde memoria no volátil, como requiere la publicación RFC 3315.

En este ejemplo, el archivo de inclusión *nxd_dhcpv6_server.h* se incluye en la línea 7. A continuación, se crea el subproceso de aplicación de servidor DHCPv6 NetX Duo en la línea 81 en el código de ejemplo siguiente. Tenga en cuenta que el bloque de control DHCPv6 “*dhcp_server_0*” se definió anteriormente como una variable global en la línea 19.

Antes de crear la instancia del servidor DHCPv6 NetX Duo, la demostración crea un grupo de paquetes para enviar mensajes DHCPv6 en la línea 84, crea una interfaz de subprocesos IP en la línea 102 y habilita UDP en NetX Duo en la línea 116.

La creación correcta del servidor DHCPv6 NetX Duo en la línea 136 incluye las dos funciones de devolución de llamada opcionales que se describen en el capítulo 1. Posibilita IPv6 e ICMPv6 necesarios para que NetX Duo procese las operaciones de IPv6 y DHCPv6 en las líneas 162 y 163. Antes de que el subproceso del servidor DHCPv6 esté listo para ejecutarse, dicho servidor debe validar su dirección IPv6 (líneas 167 a 180) y definir su interfaz DHCPv6 en las líneas 208 y 209. Se llama al servicio *nx_dhcpv6_set_server_duid* para crear el servidor si no se ha creado previamente ningún DUID de servidor en la línea 266. El servidor configura un intervalo de direcciones IP para crear una lista de direcciones asignables. Si los datos se guardan de una sesión anterior, recupera los registros de cliente y los datos de concesión IPv6 de la memoria en las líneas 283 a 318. También crea su DUID de servidor o, si se ha creado previamente, recupera los datos de DUID del almacenamiento especificado por el usuario. Esto es necesario para reproducir un DUID de servidor coherente en los reinicios. Opcionalmente, la aplicación host define un servidor DNS para los clientes que solicitan la configuración del servidor DNS.

Después, el host inicia el servidor DHCPv6 en la línea 329. Esto crea el socket UPD del servidor DHCPv6 y activa los temporizadores del servidor DHCPv6 NetX Duo. A continuación, el servidor espera para recibir las solicitudes de cliente. Aunque puede atender a muchos clientes, solo puede procesar una única solicitud de cliente a la vez.

El resto del ejemplo contiene implementaciones de host para guardar y recuperar tablas de servidor de su grupo de direcciones IPv6 asignables y registros de cliente a y desde memoria no volátil, respectivamente. También contiene un controlador de opciones para las opciones que los clientes DHCPv6 solicitan que no son compatibles directamente con el servidor DHCPv6 NetX Duo (actualmente se admite la opción de servidor DNS). Por último, hay un código para mostrar cómo guardar y recuperar "tiempo no volátil" mediante el que el servidor realiza un seguimiento de la expiración de la concesión de direcciones IP asignada.
...
**Figura 6. Aplicación de servidor DHCPv6 avanzado NetX Duo**