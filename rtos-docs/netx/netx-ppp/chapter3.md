...
Este capítulo contiene una descripción de todos los servicios PPP de Azure RTOS NetX (enumerados a continuación) en orden alfabético.
...
Este servicio crea una instancia de PPP para la instancia de IP de NetX especificada.
...
Este servicio recupera la dirección IP de DNS proporcionada por el elemento del mismo nivel. Si el elemento del mismo nivel no proporcionó ninguna dirección IP, se devuelve una dirección IP de 0.

### <a name="input-parameters"></a>Parámetros de entrada

- **ppp_ptr**: puntero al bloque de control de PPP.
- **dns_address_ptr**: destino de la dirección IP del servidor DNS.
...
Este servicio configura las direcciones IP local y del mismo nivel para su uso en el Protocolo de control de protocolo de Internet (IPCP). La aplicación PPP debe invocar este servicio en una instancia de PPP con direcciones IP válidas para sí mismo y para el otro nodo del mismo nivel.  Si no se registran direcciones válidas con una instancia de PPP, debe basarse en el homólogo PPP para definir su dirección IP.
...
Este servicio envía una solicitud de ping de LCP y establece una marca que indica que el dispositivo PPP está esperando una respuesta de eco. El servicio vuelve en cuanto se envía la solicitud. No espera una respuesta. 
