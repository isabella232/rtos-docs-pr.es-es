...
Este capítulo contiene una descripción de todos los servicios de NetX HTTP (se enumeran a continuación) por orden alfabético.
...
- nx_http_client_get_start *Iniciar una solicitud HTTP GET*
- nx_http_client_get_start_extended *Iniciar una solicitud HTTP GET*
- nx_http_client_get_packet *Obtener el siguiente paquete de datos de recursos*
- nx_http_client_put_start *Iniciar una solicitud HTTP PUT*
- nx_http_client_put_start_extended *Iniciar una solicitud HTTP PUT*
...
### <a name="create-an-http-client-instance"></a>Creación de una instancia de cliente HTTP
...
- **pool_ptr** Puntero al grupo de paquetes predeterminado. Tenga en cuenta que los paquetes de este grupo deben tener una carga útil lo suficientemente grande como para controlar el encabezado de respuesta completo. Se define mediante NX_HTTP_CLIENT_MIN_PACKET_SIZE en *nx_http_client.h*.
...
### <a name="delete-an-http-client-instance"></a>Eliminación de una instancia de cliente HTTP
...
### <a name="start-an-http-get-request"></a>Inicio de una solicitud HTTP GET
...
Este servicio intenta enviar una solicitud GET para el recurso especificado por el puntero "Recurso" en la instancia de cliente HTTP creada anteriormente. Si esta rutina devuelve NX_SUCCESS, la aplicación puede realizar varias llamadas a *nx_http_client_get_packet* para recuperar los paquetes de datos correspondientes al contenido del recurso solicitado.
...
Este servicio está en desuso. Se recomienda a los desarrolladores migrar a *nx_http_client_get_start_extended()* .
...
-**wait_option** Define cuánto tiempo esperará el servicio a la solicitud de inicio del cliente HTTP. Las opciones de espera se definen de la siguiente forma:
  ...
  - **TX_WAIT_FOREVER** (0xFFFFFFFF)<br />Si se selecciona TX_WAIT_FOREVER, el subproceso que realiza la llamada se suspende indefinidamente hasta que el servidor HTTP responde a la solicitud.<br />Si se selecciona un valor numérico (0x1-0xFFFFFFFE), se especifica el número máximo de tics del temporizador que permanecen suspendidos mientras se espera la respuesta del servidor HTTP.

**Valores devueltos**

- **NX_SUCCESS** (0x00) Mensaje de inicio de solicitud GET de cliente HTTP enviado correctamente.
- **NX_HTTP_ERROR** (0xE0) Error de cliente HTTP interno.
...
### <a name="get-next-resource-data-packet"></a>Obtención del siguiente paquete de datos de recursos
...
### <a name="start-an-http-put-request"></a>Inicio de una solicitud HTTP PUT 
...
Este servicio está en desuso. Se recomienda a los desarrolladores migrar a *nx_http_client_put_start_extended()* .
...
- **(0xF1) El nombre de usuario es demasiado grande para el búfer**
- **NX_HTTP_NOT_READY** (0xEA) El cliente HTTP no está preparado.
- NX_PTR_ERROR (0x07) Entrada de puntero no válida.
...
### <a name="send-next-resource-data-packet"></a>Envío del siguiente paquete de datos de recursos
...
Este servicio intenta enviar el siguiente paquete de contenido de recursos al servidor HTTP. Observe que se debe llamar a esta rutina repetidamente hasta que la longitud combinada de los paquetes enviados sea igual al valor de "total_bytes" especificado en la llamada a *nx_http_client_put_start()* anterior.
...
- **NX_HTTP_NOT_READY** (0xEA) El cliente HTTP no está preparado.
- **NX_HTTP_REQUEST_UNSUCCESSFUL_CODE** (0xEE) Código de error del servidor recibido.** -**NX_HTTP_BAD_PACKET_LENGTH** (0xED) Longitud de paquete no válida.
- **NX_HTTP_AUTHENTICATION_ERROR** (0xEB) Nombre o contraseña no válidos.
...
### <a name="set-the-connection-port-to-the-server"></a>Establecimiento del puerto de conexión al servidor
...
- **NX_INVALID_PORT** (0x46) El puerto supera el máximo (0xFFFF) o es cero.
- NX_PTR_ERROR (0x07) Entrada de puntero no válida.
...
### <a name="set-the-callback-to-retrieve-url-max-age-and-date"></a>Establece la devolución de llamada para recuperar la antigüedad máxima y la fecha de la dirección URL
...
### <a name="send-data-from-callback-function"></a>Envío de datos desde la función de devolución de llamada
...
### <a name="create-a-response-header-in-a-callback-function"></a>Creación de un encabezado de respuesta en una función de devolución de llamada
...
### <a name="send-an-http-packet-from-callback-function"></a>Envío un paquete HTTP desde la función de devolución de llamada
...
### <a name="send-response-from-callback-function"></a>Envío de una respuesta desde la función de devolución de llamada
...
Este servicio está en desuso. Se recomienda a los desarrolladores migrar a *nx_http_server_callback_response_send_extended()* .
...
Este servicio reemplaza a *nx_http_server_callback_response_send()* . Esta versión toma la información de longitud como argumento de entrada.
...
### <a name="get-content-from-the-request"></a>Obtención de contenido de la solicitud
...
### <a name="get-content-from-the-requestsupports-zero-length-content-length"></a>Obtención de contenido de la solicitud/admite la longitud del contenido de longitud cero
...
### <a name="get-length-of-content-in-the-request"></a>Obtención de la longitud del contenido de la solicitud
...
### <a name="get-length-of-content-in-the-requestsupports-content-length-of-zero-value"></a>Obtención de la longitud del contenido de la solicitud/admite la longitud del contenido de cero
...
### <a name="create-an-http-server-instance"></a>Creación de una instancia de servidor HTTP
...
### <a name="delete-an-http-server-instance"></a>Eliminación de una instancia de servidor HTTP
...
### <a name="retrieve-the-location-and-length-of-entity-data"></a>Recuperación de la ubicación y la longitud de los datos de entidad
...
Tenga en cuenta que NX_WEB_HTTP_MULTIPART_ENABLE debe estar habilitado para usar este servicio.

Para más información, consulte *nx_web_http_server_get_entity_header*.
...
### <a name="retrieve-the-contents-of-entity-header"></a>Recuperación del contenido del encabezado de entidad.
...
### <a name="set-the-callback-to-obtain-gmt-date-and-time"></a>Establecimiento de la devolución de llamada para obtener la fecha y hora GMT
...
### <a name="set-the-callback-to-to-handle-invalid-userpassword"></a>Establecimiento de la devolución de llamada para controlar la contraseña o el usuario no válidos
...
### <a name="set-additional-mime-maps-for-html"></a>Establecimiento de asignaciones MIME adicionales para HTML 
...
Este servicio permite al desarrollador de aplicaciones HTTP agregar tipos MIME adicionales de los tipos MIME predeterminados proporcionados por el servidor NetX HTTP (consulte *nx_http_server_get_type* para ver una lista de tipos definidos).
...
### <a name="extract-content-length-and-set-pointer-to-start-of-data"></a>Extracción de la longitud del contenido y establece el puntero en el inicio de los datos
...
Tenga en cuenta que no se debe llamar a esta función antes que a *nx_web_http_server_get_entity_header()* , ya que modifica el puntero de anteposición más allá del encabezado de entidad.
...
### <a name="receive-the-next-http-packet"></a>Recepción del siguiente paquete HTTP
...
### <a name="get-parameter-from-the-request"></a>Obtención del parámetro de la solicitud
...
### <a name="get-query-from-the-request"></a>Obtiene la consulta de la solicitud
...
### <a name="start-the-http-server"></a>Inicio del servidor HTTP
...
### <a name="stop-the-http-server"></a>Detención del servidor HTTP
...
### <a name="extract-file-type-from-client-http-request"></a>Extracción del tipo de archivo de la solicitud HTTP del cliente
...
Si se proporciona, también buscará un conjunto definido por el usuario de asignaciones MIME adicionales. Consulte *nx_http_server_mime_maps_addtional_set()* para obtener más detalles sobre las asignaciones definidas por el usuario.
...
- **Longitud de la cadena en bytes** Un valor distinto de cero es correcto.
- **Cero indica un error**
...
Para un ejemplo más detallado, consulte la descripción de
...
Tamaño del búfer de *http_type_string*.

**Valores devueltos**

- **Longitud de la cadena en bytes** Un valor distinto de cero es correcto.<br />Cero indica un error.
...
### <a name="set-digest-authenticate-callback-function"></a>Establecimiento de la función de devolución de llamada de autenticación implícita
...
### <a name="set-authentication-checking-callback-function"></a>Establecimiento de la función de devolución de llamada de comprobación de autenticación
...
- **http_server_ptr** Puntero a la instancia de servidor HTTP.
- **authentication_check_extended** Puntero a la comprobación de autenticación de la aplicación.
