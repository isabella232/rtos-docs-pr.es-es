...
- **nx_auto_ip.c**: archivo de código fuente de C para AutoIP de NetX
- **demo_netx_auto_ip.c**: demo de archivo de código fuente de C para AutoIP de NetX
...
> [!NOTE]
> Dado que AutoIP usa servicios ARP de NetX, ARP debe estar habilitado con la llamada *nx_arp_enable* antes de usar AutoIP.

## <a name="small-example-system"></a>Pequeño sistema de ejemplo

Un ejemplo de lo fácil que es usar AutoIP de NetX se describe en la figura 1.1 que aparece a continuación. En este ejemplo, el archivo de inclusión de AutoIP *nx_auto_ip.h* se incluye en la línea 002. A continuación, se crea la instancia de AutoIP de NetX en "*tx_application_define*" en la línea 090. Tenga en cuenta que el bloque de control de AutoIP de NetX "auto_ip_0" se definió anteriormente como una variable global en la línea 015. Después de la creación correcta, se inicia una AutoIP de NetX en la línea 098. El procesamiento de la función de devolución de llamada de cambio de dirección IP comienza en la línea 105, que se usa para controlar los conflictos subsiguientes o la posible resolución de direcciones DHCP.

> [!NOTE]
> En el siguiente ejemplo se supone que el dispositivo host es un dispositivo de host único. En el caso de un dispositivo de host múltiple, la aplicación host puede usar el servicio AutoIP de NetX *nx_auto_ip_interface_* establecido para especificar una interfaz de red secundaria para sondear una dirección IP. Consulte la **Guía de usuario de NetX** para obtener más detalles sobre la configuración de aplicaciones de host múltiple. Tenga en cuenta que la aplicación host debe usar la API de NetX *nx_status_ip_interface_check* para comprobar que AutoIP haya obtenido una dirección IP.
...
Hay varias opciones de configuración para compilar AutoIP para NetX. A continuación se muestra una lista de todas las opciones, donde cada una de ellas se describe en detalle:
